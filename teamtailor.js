(function() {
    ...
    const INTERVIEW_FORM_NAME = 'Entretien technique Dev'
    ...
    function prepareQuestion(question) {
        ...
        if(questionDetail && questionDetail.innerText) {
            state.push({
                ...
                description: questionDetail.innerText.split(`[${QUESTIONS_TAG}]`)[0].trim(),
    ...
    function setEchelle (item) {
        ...
        responses.forEach((response, idx) => {
            const isLegendAvailable = ['rookie', 'advanced', 'senior', 'team leader', 'business leader'].includes(level) && item.legends[level].length > 0
    ...
    function getLevel() {
        return document.querySelector(SELECTOR_CHECKED_LEVEL)?.parentElement?.innerText?.toLowerCase()
    }
